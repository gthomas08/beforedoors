...
const PERMANENT_STATUSES = new Set([400, 401, 404, 405, 410, 413, 414, 415, 422]);
...
export async function agentmailFetch(path: string, opts: FetchOptions): Promise<unknown> {
  ...
  if (!response.ok) {
    ...
    const body =
      text.length > ERROR_BODY_LIMIT ? text.slice(0, ERROR_BODY_LIMIT) + "... [truncated]" : text;
