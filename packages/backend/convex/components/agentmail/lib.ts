...
import { vRuntimeConfig, vSendKind, vSendPayload, type AgentMailEvent } from "./shared.js";
...
export const getThread = action({
  ...
  handler: async (_ctx, args) => {
    return await agentmailFetch(`/inboxes/${args.inboxId}/threads/${args.threadId}`, {
      method: "GET",
    });
  },
});
...
export const getMessage = action({
  ...
  handler: async (_ctx, args) => {
    return await agentmailFetch(`/inboxes/${args.inboxId}/messages/${args.messageId}`, {
      method: "GET",
    });
  },
});
...
export const handleEvent = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.config.onMessageReceived && event.event_type === "message.received") {
      const handle = args.config.onMessageReceived.fnHandle as FunctionHandle<
        "mutation",
